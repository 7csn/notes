...
# 事务隔离级别
    # 0 未提交读（read uncommitted）：事务未提交修改对其他事务可见。
    # 1 已提交读（read committed）：事务从开始到提交前，相关修改对其他事务不可见；前后执行相同查询可能得到不同的结果（期间其他事务提交了修改），也叫不可重复读（nonrepeatable read）。
    # 2 可重复读（repeatable read）：事务期间，对其他事务提交的 update 不可见，但是对其他事务提交的 insert/delete 可见。
    # 3 可串行化（serializable）：强制事务串行执行，每次读需要获得表级共享锁，读写相互都会阻塞。
